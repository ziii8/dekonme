...
function buildShareUrl(id, title) {
  ...
  // Utilisation d'un chemin absolu compatible avec les redirections Netlify
...
window.toggleFavorite = async function (id, btnEl) {
  ...
  if (btnEl) {
    ...
    const svgPath = btnEl.querySelector("svg");
    if (svgPath) svgPath.setAttribute("fill", !already ? "currentColor" : "none");
  }
};
...
async function updatePresence() {
  ...
  const user = await getCurrentUser();
  if (!user) return;
  await window.db.from("profiles").update({ last_seen: new Date().toISOString() }).eq("id", user.id);
}
...
function getPresenceStatus(lastSeen) {
  ...
  const diffMin = Math.block = Math.floor(diffMs / 60000);
...
/* ==================== COMPOSANTS INTERFACE GRAPHIQUE = ] ==================== */
...
function productCardHTML(l, fav) {
  ...
  // Utilisation de data-src pour s'aligner de façon optimale avec lazy-load.js
  return `
    <div class="product-card" onclick="goToProduct(${l.id})">
      ...
      <div class="product-info">
        ...
        <p class="meta">${l.city || "Lomé"} · ${l.condition}</p>
      </div>
    </div>
  `;
}
...
function renderBottomNav(active) {
  ...
  const items = [
    ...
    { key: "categories", href: "/category.html", label: "Catégorie", icon: "M4 4h7v7H4zM13 4h7v7h-7zM4 13h7v7H4zM13 13h7v7H4z" },
...
/* ==================== INTERRUPTEUR DE THÈME (DARK / LIGHT) ==================== */
...
// Initialisation au chargement immédiat du script
applyTheme();
  ...
  window.addEventListener("load", () => {
    navigator.serviceWorker
      ...
      .then(reg => {
        ...
        // Vérification de mise à jour toutes les heures (3600000 ms)
